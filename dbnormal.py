...
def check4doub(checkboard,numb):
    for numb in checkboard:
        if checkboard.count(numb) != 1:
            checkboard.pop(checkboard.index(numb))
    ...
    while x <= reps:
        ...
        for key in checkboard:
            if comparelists(checkboard,lst2check):
                break
            if in2dlist(key, left):  # if keys is in left
                for cell in left:
                    for val in cell:
...
def findnm(left,right,cands,num):
    ...
    if nf2:
        for val in inboth:
            ...
            if ispartof2dlist(val,cands) == 0 and ispartof2dlist(right[leftind[tmpindex]],cands) == 2 and ispartof2dlist(left[rightind[tmpindex]],cands) == 0:
                nf3 = False
        if nf3:
            for var in left :
